...
volatile int positionIndex  = 6082;//3041;//maxCount/2;    // Used for tracking number of steps the motor has taken
volatile int positionTarget = 6082;//3041;//maxCount/2;   // Assume we start in the middle of playing area
        byte b;
volatile int movingFlag     = 1;                // Flag to determine if system is moving or not
...
/*
  ...
  if ( positionIndex == positionTarget ) { // missed one step?
    movingFlag = 0;
    frequency_PWM = 20000;    // Reset to initial frequency (velocity)
  }
    ...
    //if( PWM_flag == false ){
      positionIndex++;
    //}
    frequency_PWM = frequency_PWM + 4;    // accelerate
  }
...
*/  
    ...
    if( frequency_PWM >= 0 && frequency_PWM < 60000 ){
...
/*
 // Incrementally increase PWM frequency (accelerate)
 if( frequency_PWM < 60000 && PWM_flag == HIGH){
    frequency_PWM = frequency_PWM + 1;
    positionIndex++;
    startTimer(TC2, 0, TC6_IRQn, frequency_PWM );
 }
 else{
   // don't increase frequency_PWM
 }
*/
...
/*
void TC2_Handler() {
  TC_GetStatus(TC0, 2); // Accept interrupt

  // Increase speed (PWM frequency)
  if( frequency_PWM >= 0 && frequency_PWM < 30000 ){
    frequency_PWM = frequency_PWM + 10;
  }
  else{
    // do not change frequency
  }
...
void loop() {
  ...
  if( (micros() - timeMicro) > ((1/(float)frequency_PWM))*1000000.0 ){
  ...
  if ( movingFlag == 0 ) {
    ...
    delayMicroseconds(((1/(float)frequency_PWM))*1000000.0);
    digitalWrite(pinPWM, PWM_flag = !PWM_flag);
